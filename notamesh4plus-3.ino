...
void setup() {
  ...
  random16_add_entropy(analogRead(2));
    ...
    NUM_LEDS = EEPROM.read(STRANDLEN);                                            //Востановим количество светодиодов
    #if MAX_LEDS < 255
          ...
          ((ledMode> maxMode) && (ledMode != 100) ) )      
    {
      ...
      switch (demorun)  {
        ...
        default : ledMode = INITMODE; break;
      }
    #else
      ledMode = INITMODE;
    #endif 
...
void loop() {
  ...
  EVERY_N_MILLIS_I(thistimer, thisdelay) {                                    // Sets the original delay time.
    ...
    if ( StepMode< NUM_LEDS) {                                                  // есть шаги, исполняем
        uint16_t change_time = (1000L *CHANGE_TIME) / NUM_LEDS;                 // время в мСек на каждый светодиод
        if (change_time < 20) change_time = 20;
//        static uint8_t change_increment = (uint32_t)change_time * NUM_LEDS / (1000L *CHANGE_TIME)+1;
